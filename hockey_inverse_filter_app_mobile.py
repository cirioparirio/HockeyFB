...
def identify_columns(data_df):
    ...
    filter_columns = [
        ...
        'Somma Poiss',  # Aggiunta la nuova colonna "Somma Poiss"
    ...
    if len(result_columns) == 0:
        ...
        numeric_result_cols = [28, 29, 31, 32, 34, 35, 36, 37, 38, 39, 41, 42, 43, 44, 45]  # Aggiunto indice 45 per OVER 6,5
            ...
            if st.button("Esegui Filtraggio Inverso", type="primary", use_container_width=True):
                ...
                with st.spinner(f"Ricerca delle combinazioni di filtri per {target_column} > {target_percentage}% con almeno {min_matches} partite..."):
                    ...
                    if not results:
                        ...
                        # Aggiungi un pulsante per scaricare i grafici
                        buffer = BytesIO()
                        percentage_chart.savefig(buffer, format='png', dpi=300)
                        buffer.seek(0)
                        st.download_button(
                            label="Scarica Grafico Percentuali",
                            data=buffer,
                            file_name="grafico_percentuali.png",
                            mime="image/png"
                        )
                        
                        buffer = BytesIO()
                        matches_chart.savefig(buffer, format='png', dpi=300)
                        buffer.seek(0)
                        st.download_button(
                            label="Scarica Grafico Partite",
                            data=buffer,
                            file_name="grafico_partite.png",
                            mime="image/png"
                        )

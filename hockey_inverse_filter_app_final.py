...
# Importazione condizionale di matplotlib e seaborn
# Non mostriamo avvisi qui per evitare chiamate Streamlit prima di set_page_config
...
try:
    ...
    pass  # Gestiamo silenziosamente l'errore qui, mostreremo avvisi solo quando necessario
...
# Mostra un avviso se matplotlib non è disponibile
if not matplotlib_available:
    st.warning("Matplotlib o Seaborn non sono disponibili. I grafici non saranno visualizzati.")
            ...
            if st.button("Esegui Filtraggio Inverso", type="primary", use_container_width=True):
                ...
                with st.spinner(f"Ricerca delle combinazioni di filtri per {target_column} > {target_percentage}% con almeno {min_matches} partite..."):
                    ...
                    if not results:
                        ...
                        # Crea e mostra i grafici solo se matplotlib è disponibile
                        if matplotlib_available:
                            st.subheader("Grafici")
                            
                            # Grafico delle percentuali
                            st.write("Percentuali di Successo")
                            percentage_chart = create_percentage_chart(results_df)
                            if percentage_chart:
                                st.pyplot(percentage_chart)
                            
                            # Grafico del numero di partite
                            st.write("Numero di Partite")
                            matches_chart = create_matches_chart(results_df)
                            if matches_chart:
                                st.pyplot(matches_chart)
                        else:
                            st.info("I grafici non sono disponibili perché matplotlib non è installato correttamente.")
